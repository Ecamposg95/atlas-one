...
export function CashCutsTab({ params, page, onPageChange, onRowClick }: Props) {
  ...
  const columns: MoneyColumn<CashCutRow>[] = [
    ...
    { key: 'shortage_total', label: 'Faltante', align: 'right', render: (r) => <span className="pv2-tone-down">{money(r.shortage_total)}</span> },
    { key: 'overage_total', label: 'Sobrante', align: 'right', render: (r) => money(r.overage_total) },
    { key: 'avg_abs_difference', label: 'Dif. prom.', align: 'right', render: (r) => money(r.avg_abs_difference) },
