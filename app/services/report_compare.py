...
def compare_window(start: datetime, end: datetime, mode: str) -> Optional[tuple[datetime, datetime]]:
    ...
    - `prev`: la ventana inmediatamente anterior de la misma duración; termina
      donde empieza la actual (no se solapan ni dejan hueco).
...
def decorate_with_previous(
    ...
    *,
    ...
    Empareja por `key`. Una fila sin par previo lleva `prev_* = "0.00"` y
    ...
    for row in items:
        ...
        for field in fields:
            ...
            new_row[f"prev_{field}"] = str(prev_dec.quantize(Decimal("1.00")))
